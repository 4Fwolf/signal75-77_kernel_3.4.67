...
static int acc_timer_state;

static void acc_timer_exit() 
{ 
    acc_timer_state = 0; 
    del_timer(&acc_timer);   
}

static void acc_timer_function() 
{ 
    acc_timer_exit(); 
    if (acc_timer_state == 0)
    {
...
static void acc_timer_init() 
{ 
    if (acc_timer_state != 1)
    {
      acc_timer_state = 1;
      init_timer(&acc_timer); 
      acc_timer.data= 5; 
      acc_timer.expires = jiffies + (2*HZ);  
      acc_timer.function = acc_timer_function; 
      add_timer(&acc_timer); 
    }  
}
// end Zormax added.
...
void switch_NTSC_to_PAL(int mode)
{
    ...
    else
    {
        ...
        //struct tv_mode_settings* cust_tv_settings = get_tv_mode_settings(); // IICuX
...
void accdet_eint_work_callback(struct work_struct *work)
{
    ...
    accdet_init();
	//enable ACCDET unit
//    enable_accdet(ACCDET_SWCTRL_EN); // IICuX
...
static inline int accdet_setup_eint(void)
{
	...
	mt_eint_registration(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_DEBOUNCE_EN, CUST_EINT_ACCDET_POLARITY, accdet_eint_func, 0);
...
static inline void check_cable_type(void)
{
   ...
   // clear_accdet_interrupt();
    ...
    switch(accdet_status)
    {
        ...
        // Zormax added. Send key events while not in-call.
        #ifdef HEADSET_MUSIC_KEY
		if((call_status == 0) && button_status)
		{	
		    if(is_long_press() && (acc_timer_state != 1))
		{
                        input_report_key(kpd_accdet_dev, KEY_NEXTSONG, 1);
                        input_report_key(kpd_accdet_dev, KEY_NEXTSONG, 0);
                        input_sync(kpd_accdet_dev);
		         ACCDET_DEBUG("[Accdet] long press remote button to next song!\n");			
                                                }
                    else 
							   {
		        if (acc_timer_state == 1)
                          ...
                          input_sync(kpd_accdet_dev);	
			   ACCDET_DEBUG("[Accdet] short press remote button to previous song!\n");			  
                                                }
                                                else
		        {
		          acc_timer_init();
...
//	&driver_attr_accdet_pin_recognition, // IICuX
};
...
//	cust_headset_settings = get_cust_headset_settings();	// IICuX	
...
//	long_press_time = press_key_time->headset_long_press_time; // IICuX
